...
import static online.luismartinsdev.mediaplayer.util.Constants.Resources.IMAGES_ICONS_NEXT_ICON_PNG;

@View
public class NextButton extends Button implements Cleaner.Cleanable {

    private final ChangeListener<Media> activeSongChangeListener;
    private final String iconUrl;
    private final MediaPlayerViewState state;

    public NextButton(MediaPlayerViewState state,
                      MediaPlayerController.HandlerNextAction handlerNextAction) {
        this.state = state;
        iconUrl = requireNonNull(this.getClass().getClassLoader()
                .getResource(IMAGES_ICONS_NEXT_ICON_PNG)).toString();
        ImageView imageView = createButtonImage();

        setOnAction(handlerNextAction);
